...
Changelog (round 2 - user-reported bugs):
  1) AHB detection was a single description-wide boolean, so enabling Azure
     Hybrid Benefit for SQL alone was also silently assumed to mean AHB for
     Windows OS, suppressing the real Windows license charge and dumping it
     into the SQL license line via the unaccounted-cost subtraction.
     Fixed: `detect_ahb_scoped()` scopes AHB independently to the Windows-OS
     segment vs. the SQL-Server segment of the description (using a new
     paren-aware `_split_license_segments()` splitter, since a naive comma
     split breaks apart phrases like "SQL Standard (vCPU License, Bring your
     own license)"). Each license is now charged/suppressed independently.
  2) VMs always priced a fixed 730-hour month, ignoring a custom "hours per
     month" selection (e.g. 400h) in the Calculator.
     Fixed: `extract_hours()` parses a custom-hours figure from the row
     description when present, and it flows through `_hourly_to_monthly()`
     into compute/Windows-license/Premium-OS PAYG. Reserved Instance pricing
     is deliberately NOT scaled by custom hours (an RI is a fixed monthly
     commitment regardless of runtime) - this is called out in Remarks.
  3) App Service, AKS, Cosmos DB (and several other PaaS services) had no
     entry in SVC_MAP and fell into the generic "Others" sheet.
     Fixed: SVC_MAP and SHEET_ORDER extended with dedicated entries for App
     Service, AKS, Cosmos DB, Azure Database for MySQL/PostgreSQL, Redis
     Cache, Functions, and Container Instances.
  4) AKS and App Service never got Reserved pricing - only "Virtual Machines"
     and "Virtual Machine Scale Sets" were routed through the RI-enrichment
     pipeline; everything else just echoed the source file's PAYG/RI figures
     verbatim (or PAYG duplicated into the RI columns).
     Fixed: AKS nodes are billed/reserved exactly like standalone VMs of the
     same size, so "AKS" was added to VM_LIKE_SHEETS to reuse
     enrich_vms_concurrent() as-is. App Service does NOT use VM Reserved
     Instances - it has its own "Reserved App Service Plan" SKU space - so a
     dedicated `get_app_service_reservation_pricing()` /
     `enrich_app_service_concurrent()` pipeline was added, querying
     serviceName='Azure App Service' priceType='Reservation'. Only Premium
     v2/v3 tiers are reservable; Free/Shared/Basic/Standard tiers are
     detected and clearly flagged as "not reservable" rather than silently
     given fake RI pricing.

  NOTE ON UNVERIFIED REGEXES: extract_hours() (custom-hours text) and
  extract_app_service_plan_sku() (App Service tier text) and
  extract_vm_sku()/extract_quantity() as applied to AKS description rows
  are all best-effort pattern matches against Azure Pricing Calculator
  export text. They were written from the general shape of Calculator
  exports, not from a real sample of these specific row types - verify
  against a real export and adjust the regex if a row doesn't match (a
  failure is always surfaced via Remarks, never silent).
"""
import re
import sys
import logging
...
SVC_MAP = {
    ...
    "cosmos db": "Cosmos DB",
    "azure database for mysql": "Azure Database (MySQL/PostgreSQL)",
    "azure database for postgresql": "Azure Database (MySQL/PostgreSQL)",
    "azure database for mariadb": "Azure Database (MySQL/PostgreSQL)",
...
VM_LIKE_SHEETS = {"Virtual Machines", "Virtual Machine Scale Sets", "AKS"}

# Service-type "sheet" names priced through the App Service Reservation
# pricing pipeline (own SKU space - "Reserved App Service Plan", not VM
# Reserved Instances - so it needs its own enrichment function, but reuses
# the same "standalone" row rendering as SQL/standalone VM rows).
APP_SERVICE_SHEETS = {"App Service"}

SHEET_ORDER = [
    "Virtual Machines", "Virtual Machine Scale Sets", "AKS", "App Service",
    "Managed Disks", "Public IP Addresses",
    "Load Balancer", "Application Gateway", "Azure Firewall", "VPN Gateway",
    "Storage Accounts", "Azure Backup", "SQL", "Cosmos DB",
    "Azure Database (MySQL/PostgreSQL)", "Redis Cache", "Functions",
    "Container Instances", "Bandwidth", "Azure Monitor", "Key Vault", "Others",
]
...
def _hourly_to_monthly(price, hours=730.0):
    """Convert an hourly retail price to a monthly figure using the row's
    actual selected runtime hours (from the Pricing Calculator's 'Hours per
    month' custom-hours option), defaulting to a full 730-hour month when no
    custom hours are present in the description."""
    return price * hours

def extract_hours(desc):
    """
    Detect a custom 'hours per month' selection from the row description.

    The Azure Pricing Calculator embeds the selected runtime as free text
    when it differs from the full-month default, e.g. '... x 400 Hours' or
    '(400 Hours)'. If nothing matches, assume the full 730-hour month (this
    preserves prior behaviour for the common "run continuously" case).

    NOTE: the exact substring the Calculator emits for custom hours can vary
    by export template/version - this regex should be verified against a
    real custom-hours export and adjusted if it doesn't match.
    """
    if not desc:
        return 730.0
    m = re.search(r'(\d+(?:\.\d+)?)\s*Hours?\b', desc, re.IGNORECASE)
    if m:
        try:
            hours = float(m.group(1))
            if 0 < hours <= 744:  # sanity bound - a calendar month has at most 744 hours
                return hours
        except ValueError:
            pass
    return 730.0
...
def detect_ahb_scoped(desc):
    """
    Azure Hybrid Benefit / BYOL can be applied to the Windows OS license and
    the SQL Server license INDEPENDENTLY - a VM can have AHB on SQL only, on
    Windows only, on both, or on neither. A single description-wide boolean
    (the old `detect_ahb`) cannot tell these apart, and previously caused a
    VM with "SQL ... (Bring your own license)" (AHB on SQL only) to also
    silently suppress the Windows license charge, and to dump the real
    Windows license cost into the SQL license line via the unaccounted-cost
    subtraction.

    Returns (ahb_windows, ahb_sql) - each True only if an AHB/BYOL keyword
    appears in that specific comma/semicolon-separated segment of the
    description (the same paren-aware segmentation get_exact_license_name()
    uses, so 'SQL Standard (vCPU License, Bring your own license)' is kept
    as one segment and correctly scoped to SQL rather than falling through
    to "unscoped").
    """
    parts = _split_license_segments(desc)
    ahb_windows = ahb_sql = False
    ahb_unscoped = False
    for p in parts:
        p_l = p.lower()
        if not any(kw in p_l for kw in AHB_KEYWORDS):
            continue
        if "sql" in p_l:
            ahb_sql = True
        elif "windows" in p_l:
            ...
            # AHB keyword present but not clearly scoped to either license in
            # its own segment (e.g. a single combined description string) -
            # don't guess; caller decides how to handle/flag this.
            ahb_unscoped = True
    if ahb_unscoped and not ahb_windows and not ahb_sql:
        # Fall back to the old conservative behaviour (apply to both) but the
        # caller adds a Remarks note so this assumption is never silent.
        return True, True, True
    return ahb_windows, ahb_sql, False
...
def _split_license_segments(desc):
    """
    Split a Calculator description into comma/semicolon-separated license
    segments, WITHOUT splitting on commas that occur inside parentheses -
    e.g. 'SQL Standard (vCPU License, Bring your own license)' must stay as
    ONE segment (both the SQL tag and the AHB/BYOL tag belong together),
    not be torn into 'SQL Standard (vCPU License' and 'Bring your own
    license)' by a naive re.split on every comma.
    """
    parts = []
    depth = 0
    current = ""
...
def get_vm_pricing(session, cache, sku, region_display, is_spot, currency="INR", hours=730.0):
    """
    ...
    `hours` scales the PAYG compute/Windows-license figures to the VM's
    actual selected runtime (e.g. 400 custom hours/month instead of the
    default full 730-hour month). Reserved Instance pricing is deliberately
    NOT scaled by `hours` - an RI is a fixed monthly/annual commitment billed
    in full regardless of how many hours the VM actually runs, so it always
    reflects a full-month-equivalent rate.
    """
    ...
    try:
        ...
        result["compute_payg"] = _hourly_to_monthly(linux_hr, hours) if linux_hr else None
        if win_hr and linux_hr:
            result["windows_license"] = max(0, _hourly_to_monthly(win_hr, hours) - _hourly_to_monthly(linux_hr, hours))

        if not is_spot:
            # RI monthly-equivalent is always a fixed full-term/12 or /36
            # figure - it is NOT scaled by `hours` (see docstring above).
...
def get_premium_os_license_price(session, cache, payg_items, desc, sku, region, qty, currency, hours=730.0):
    """
    ...
    `hours` scales both the live-meter and static-fallback results to the
    row's actual selected runtime (see `_hourly_to_monthly` / `extract_hours`),
    consistent with how compute/Windows-license PAYG is scaled.
    """
    desc_l = desc.lower()
    ...
    if cands:
        hourly = min(cands)
        return {"monthly": _hourly_to_monthly(hourly, hours) * qty, "approximate": False, "unavailable": False}

    # Fallback: static USD table, converted via a per-row (not global) ratio.
    # The table values are full-month (730h) reference prices, so scale them
    # by hours/730 to stay consistent with the live-meter path above.
    vcpus = extract_vcpus(desc)
    family = _static_os_family(desc_l)
    tier = "low" if (vcpus and vcpus <= 4) else "high"
    usd_price = _STATIC_PREMIUM_OS_USD[family][tier] * (hours / 730.0)

    if currency == "USD":
        return {"monthly": usd_price * qty, "approximate": True, "unavailable": False}

    native = get_vm_pricing(session, cache, sku, region, False, currency, hours)
    usd = get_vm_pricing(session, cache, sku, region, False, "USD", hours)
...
# ── App Service Reservation pricing ─────────────────────────────────────────
# App Service does NOT sell Reserved VM Instances - it sells its own
# "Reserved App Service Plan" SKUs via the same Retail Prices API, filtered
# on serviceName rather than a VM armSkuName. Only Premium v2/v3 tiers are
# reservable at all; Free/Shared/Basic/Standard tiers have no Reservation SKU
# in the API and must be shown as "not reservable", never silently defaulted
# to PAYG-as-RI.
APP_SERVICE_NON_RESERVABLE_TIERS = {"F1", "D1", "SHARED", "B1", "B2", "B3", "S1", "S2", "S3"}

def extract_app_service_plan_sku(desc):
    """
    Best-effort App Service Plan tier extractor, e.g. 'P1v3', 'P2v2', 'P3mv3'.

    NOTE: the exact wording Azure Pricing Calculator uses for App Service
    Plan rows can vary by export template - verify this regex against a real
    App Service export row and adjust if it doesn't match (this is flagged
    clearly via Remarks when it fails, never silently mispriced).
    """
    if not desc:
        return None
    # Premium v2/v3 tiers (reservable): P1v2, P2v3, P3mv3, etc.
    m = re.search(r'\b([Pp]\d+[Mm]?[Vv]\d)\b', desc)
    if m:
        return m.group(1)[0].upper() + m.group(1)[1:].lower()
    # Free/Shared/Basic/Standard tiers (NOT reservable, but still need to be
    # recognized so they're correctly flagged rather than treated as
    # "unparseable").
    m = re.search(r'\b([FfDdBbSs]\d)\b', desc)
    if m:
        return m.group(1).upper()
    m = re.search(r'\b(Shared|Free)\b', desc, re.IGNORECASE)
    if m:
        return m.group(1).capitalize()
    return None

def get_app_service_reservation_pricing(session, cache, tier_sku, region_display, currency):
    """Fetch 1yr/3yr Reserved App Service Plan monthly-equivalent pricing,
    directly in `currency`, for a Premium v2/v3 tier SKU."""
    region = arm_region(region_display)
    result = {"ri1": None, "ri3": None}

    def fetch(ptype):
        filt = (f"armSkuName eq '{tier_sku}' and armRegionName eq '{region}' "
                f"and serviceName eq 'Azure App Service' and priceType eq '{ptype}'")
        return _api(session, cache, filt, currency)

    def _min_price(items, term):
        cands = [i["retailPrice"] for i in items if i.get("reservationTerm") == term and i.get("retailPrice", 0) > 0]
        return min(cands) if cands else None

    try:
        ri_items = fetch("Reservation")
        v1 = _min_price(ri_items, "1 Year")
        if v1 is not None:
            result["ri1"] = v1 / 12
        v3 = _min_price(ri_items, "3 Years")
        if v3 is not None:
            result["ri3"] = v3 / 36
    except Exception as e:
        log.warning(f"App Service RI pricing error for tier={tier_sku} region={region} currency={currency}: {e}")

    return result

def enrich_app_service_concurrent(rows, currency="INR"):
    """
    Enrich App Service Plan rows with live Reserved App Service Plan pricing
    (the App Service equivalent of Reserved VM Instances for compute). Each
    row keeps its original PAYG figure and only the RI columns are replaced -
    App Service Plan pricing is not split into compute/license components the
    way VM pricing is.
    """
    log.info(f"Querying Azure Retail Pricing API for {len(rows)} App Service row(s), currency={currency}...")
    ...
    def process_row(row):
        ...
        try:
            ...
            tier = extract_app_service_plan_sku(desc)
            if not tier:
                _append_remark(row, "App Service Plan tier could not be parsed from description - showing original calculator values; RI pricing not separated")
                return row

            if tier.upper() in APP_SERVICE_NON_RESERVABLE_TIERS:
                _append_remark(row, f"App Service Plan tier '{tier}' has no Reserved Instance pricing (only Premium v2/v3 tiers are reservable) - PAYG shown in RI columns")
                return row

            pricing = get_app_service_reservation_pricing(session, cache, tier, region, currency)
            payg = row.get("payg", 0)

            if pricing.get("ri1") is not None:
                row["ri1"] = round(pricing["ri1"], 2)
            else:
                row["ri1"] = payg
                _append_remark(row, f"1-Year Reserved App Service Plan pricing not found for tier '{tier}' in region '{region}' - showing PAYG")

            if pricing.get("ri3") is not None:
                row["ri3"] = round(pricing["ri3"], 2)
            else:
                row["ri3"] = payg
                _append_remark(row, f"3-Year Reserved App Service Plan pricing not found for tier '{tier}' in region '{region}' - showing PAYG")

            return row
        except Exception as e:
            log.error(f"Unhandled error processing App Service row ({_row_context(row)}): {e}", exc_info=True)
            row["api"] = {"is_standalone": True}
            _append_remark(row, f"Processing error - showing original calculator values (see server logs): {e}")
            return row

    max_workers = min(10, max(2, len(rows)))
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                log.error(f"Uncaught exception escaped process_row (App Service): {e}", exc_info=True)
...
def enrich_vms_concurrent(vm_rows, currency="INR"):
    ...
    def process_row(row):
        ...
        try:
            ...
            ahb_windows, ahb_sql, ahb_ambiguous = detect_ahb_scoped(desc)
            ...
            if hours != 730.0:
                _append_remark(row, f"Custom hours ({hours:.0f}h/month) applied to PAYG figures; Reserved Instance pricing remains a fixed full-month commitment and is not reduced by custom hours")

            if ahb_ambiguous:
                _append_remark(row, "Azure Hybrid Benefit / BYOL keyword found but could not be scoped to Windows vs. SQL from the description text - applied to both as a conservative default; please verify")
            ...
            pricing = get_vm_pricing(session, cache, sku, region, is_spot, currency, hours)
            ...
            # AHB is scoped independently per license (Windows OS vs. SQL
            # Server) - a VM can have AHB applied to one, both, or neither.
            # `has_sql`/`has_premium_os` below are further narrowed to
            # "chargeable" (i.e. NOT covered by that license's own AHB flag),
            # so an AHB-on-SQL-only VM no longer also suppresses Windows, and
            # an AHB-on-Windows-only VM no longer suppresses SQL.
            win_lic_payg = win_native if (os_type == "Windows" and not ahb_windows) else 0
            if os_type == "Windows" and ahb_windows:
                _append_remark(row, "Azure Hybrid Benefit / BYOL detected for Windows OS - Windows license not charged separately")

            has_sql_chargeable = has_sql and not ahb_sql
            if has_sql and ahb_sql:
                _append_remark(row, "Azure Hybrid Benefit detected for SQL Server - SQL license not charged separately")
            ...
            if unaccounted > epsilon:
                if has_sql_chargeable and has_premium_os:
                    os_result = get_premium_os_license_price(
                        session, cache, pricing.get("raw_payg_items", []), desc, sku, region, qty, currency, hours
                    )
                ...
                elif has_sql and ahb_sql:
                    # SQL is present but AHB-covered, and nothing else claims
                    # the leftover - most likely a residual Windows/rounding
                    # amount in the source file. Don't relabel it as SQL.
                    other_payg = unaccounted
                    _append_remark(row, "Unexplained cost remaining after AHB-covered SQL license was excluded - shown as 'Other/Unidentified Cost'; please verify")
                else:
...
def convert(input_path, output_path, currency="INR"):
    ...
    for sname in APP_SERVICE_SHEETS:
        if sname in buckets:
            enrich_app_service_concurrent(buckets[sname], currency)
    ...
    for sname in SHEET_ORDER:
        ...
        if sname in VM_LIKE_SHEETS or sname in APP_SERVICE_SHEETS:
